...
must_haves:
  truths:
    ...
    - "Cargo profiles include debug symbols for profiling tools"
  artifacts:
    - path: "src/lib.rs"
      provides: "Library crate re-exporting all public modules"
      contains: "pub mod parser"
    - path: "Cargo.toml"
      provides: "Criterion dev-dependency, dhat optional dependency, profile configs"
      contains: "criterion"
  key_links:
    - from: "src/lib.rs"
      to: "src/parser.rs"
...
<objective>
Create library crate for external access, add profiling dependencies, extract panic hook, and configure Cargo profiles.

Purpose: All other Phase 14 plans (benchmarks, integration tests) require modules to be accessible via a library crate. This plan establishes that foundation plus the dhat heap profiling capability and panic hook improvement.
Output: src/lib.rs exposing modules, Cargo.toml with Criterion/dhat/profiles, refactored panic hook
</objective>
...
Run `cargo check --features dhat-heap` -- must compile with dhat feature enabled.
  ...
  <done>
src/lib.rs exists and re-exports all modules. Cargo.toml has Criterion dev-dep, dhat optional dep, bench configs, and profile configs. Both `cargo check` and `cargo check --features dhat-heap` succeed.
  </done>
</task>
...
6. main.rs uses library crate imports instead of local mod declarations
7. Panic hook is in a named function called before terminal init
</verification>
...
- Profile configs enable debug symbols for profiling
